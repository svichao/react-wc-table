...
export default function buildCrossTable(
  ...
  const leftHeaderWidth = Math.max(
    leftMetaColumns.length,
    getTreeDepth(leftTree) + 1,
  );
  ...
  /** 获取表格的列配置 */
  function getColumns(): CrossTableRenderColumn[] | any {
    ...
    function getLeftPartColumns(): CrossTableRenderColumn[] {
      ...
      for (let index = 0; index < leftHeaderWidth; index++) {
        const metaCol =
          leftMetaColumns[index] ?? ({} as CrossTableLeftMetaColumn);
      ...
      function leftHeaderGetValueFactory(
        metaCol: CrossTableLeftMetaColumn,
        colIndex: number,
      ) {
      ...
      function leftHeaderRenderFactory(
        metaCol: CrossTableLeftMetaColumn,
        colIndex: number,
      ) {
    ...
    function getDataColumn(
      ...
      return {
        ...
        getCellProps(value: any, row: CrossTableRenderRow) {
          if (options.getCellProps) {
            const leftDepth = row.nodes.length - 1;
            const leftNode = row.nodes[leftDepth];
            return options.getCellProps(
              value,
              leftNode,
              topNode,
              leftDepth,
    ...
    function dfs(nodes: LeftCrossTreeNode[], ctx: DfsCtx): { count: number } {
      ...
      for (const node of nodes) {
        ...
        if (isLeafNode(node)) {
          ...
          ctx.rects.push(rect);
          ctx.nodes.push(node);
          const ret = dfs(node.children as any, {
            nodes: ctx.nodes,
            rects: ctx.rects,
            depth: ctx.depth + 1,
