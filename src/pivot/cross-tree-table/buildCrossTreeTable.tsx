...
import { features, TablePipeline } from '../../extends/pipeline';
...
export default function buildCrossTreeTable(
  ...
  const pipeline: any = new TablePipeline({
    state: {},
    setState: noop,
  ...
  /** 获取表格的列配置 */
  function getColumns(): ArtColumn[] | any {
    /** 获取表格数据部分的列配置 */
    ...
    function getDataColumn(
      ...
      return {
        ...
        getCellProps(value: any, row: CrossTreeTableRenderRow) {
    ...
    function dfs(
      ...
      for (const node of nodes) {
        ...
        if (standardIsLeafNode(node)) {
          ...
          ctx.nodes.push(node);
          const children = dfs(node.children as any, ctx);
