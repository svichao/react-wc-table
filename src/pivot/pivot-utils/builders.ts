...
export function buildRecordMatrix({
  ...
  function buildByLeft(
    ctx: BuildingCtx,
    slice: any[],
    drillNode: DrillNode,
    depth: number,
  ): TransientMatrixRow {
    let children: TransientMatrixRow[] = null as any;
  ...
  function buildByTop(
    ...
    let children: TransientMatrixCol[] | any = null;
    let record: any;
  ...
  function mergeColsByTopTree(
    ctx: BuildingCtx,
    colsToMerge: TransientMatrixCol[],
  ): TransientMatrixCol {
    ...
    function dfs(
      ...
      if (!isLeafNode(topDrillNode)) {
        ...
        const drillChildDict = fromEntries(
          topDrillNode.children.map((child) => [child.value, child]),
        );
        const colChildDictArray = cols.map((col) =>
          fromEntries(col.children.map((child) => [child.topValue, child])),
        );
        children = topDrillNode.children.map((item) => {
          const childCols = colChildDictArray
            .map((colChildDict) => colChildDict[item.value])
            .filter(Boolean);
