...
import { CrossTableIndicator, type CrossTreeNode } from '../cross-table';
...
export function convertDrillTreeToCrossTree<
  T extends CrossTreeNode = CrossTreeNode,
>(
  drillTree: DrillNode[],
  {
    indicators,
  ...
  function dfs(drillNodes: DrillNode[], depth: number): T[] {
    ...
    for (const node of drillNodes) {
      ...
      if (isLeafNode(node) && !node.hasChild) {
        ...
        if (
          !supportsExpand ||
          (enforceExpandTotalNode && node.key === totalKey)
        ) {
          // 不支持展开功能 或是强制展开
        ...
        if (needProcessChildren) {
          ...
          if (subtotalNodeData) {
            ...
            const subtotalTreeNode: T = drillNodeToTreeNode(
              subtotalDrillNode,
              nodeData,
            );
