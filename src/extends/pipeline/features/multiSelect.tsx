...
import { always, arrayUtils, safeGetRowKey } from '../utils/utils';
...
export function multiSelect(opts: MultiSelectFeatureOptions = {}) {
  return function multiSelectStep(pipeline: TablePipeline) {
    ...
    if (Checkbox == null) {
      throw new Error(
        '使用 multiSelect 之前需要设置 pipeline.ctx.components.Checkbox',
      );
    }
    ...
    const value: string[] =
      opts.value ??
      pipeline.getStateAtKey(stateKey)?.value ??
      opts.defaultValue ??
      [];
    const lastKey: string =
      opts.lastKey ??
      pipeline.getStateAtKey(stateKey)?.lastKey ??
      opts.defaultLastKey ??
      '';
    const onChange: MultiSelectFeatureOptions['onChange'] = (
      nextValue,
      key,
      keys,
      action,
    ...
    const isAllChecked =
      allKeys.length > 0 && allKeys.every((key) => set.has(key));
      ...
      <Checkbox
        ...
        onChange={(_: any) => {
          if (isAllChecked) {
            ...
            onChange(
              arrayUtils.merge(value, allKeys),
              '',
              allKeys,
              'check-all',
            );
    ...
    const checkboxColumn: ArtColumn = {
      ...
      cellRenderer: ({ rowData, rowIndex }) => {
        const key = safeGetRowKey(primaryKey, rowData, rowIndex);
    ...
    function onCheckboxChange(
      ...
      if (batch && lastKey) {
        ...
        const [start, end] =
          lastIdx < cntIdx ? [lastIdx, cntIdx] : [cntIdx, lastIdx];
        batchKeys = allKeys.slice(start, end + 1);
      }
      if (onChange) {
        if (prevChecked) {
          onChange(
            arrayUtils.diff(value, batchKeys),
            key,
            batchKeys,
            'uncheck',
          );
