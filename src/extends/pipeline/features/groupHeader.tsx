...
export function groupHeader(opts: {
  ...
  return (pipeline: TablePipeline) => {
    ...
    if (columns.find((p) => p.children !== null && p.children !== undefined)) {
      const flatColumns = collectNodes(columns);
      // const padding = opts?.cellPadding || 7.5;
      const colTreeInfo: Record<string, RecursiveFlatMapInfo<ArtColumn>> = {};
      pipeline.mapColumns(
        makeRecursiveMapper((col: any, args) => {
      ...
      let maxLevel = 0;
      const rootColumns: ArtColumn[] = [];
      Object.keys(colTreeInfo).forEach((key) => {
      ...
      const headerHead = opts?.headHeight ? opts.headHeight : 32;
      ...
      const headerRenderer = ({
        ...
        const renderTreeLevel = (
          ...
          const columnLevel = TreeUtils.getTreeNodeLevel([tree], column);
        ...
        columns.forEach((column: any, columnIndex: number) => {
          ...
          const tree = root;
          // const rootMaxLevel = colTreeInfo[column.key].path.length;
          // console.log('rootMaxLevel: ', rootMaxLevel);
          renderTreeLevel(
            tree,
            headerIndex,
            maxLevel,
