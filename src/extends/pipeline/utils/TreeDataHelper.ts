...
import { AbstractTreeNode, arrayUtils, isLeafNode } from './utils';
...
export default class TreeDataHelper<N extends AbstractTreeNode> {
  ...
  private initWrapperTree() {
    ...
    function dfs(
      parentWrapper: Wrapper<N>,
      nodes: N[],
      inheritParentChecked: boolean,
    ) {
  ...
  public getValueAfterUncheck(nodeValue: string) {
    ...
    const nextValue = arrayUtils.diff(
      this.value.concat(appendArray),
      removeSet,
    );
