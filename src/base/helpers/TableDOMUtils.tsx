...
export class TableDOMHelper {
  ...
  constructor(artTableWrapper: HTMLDivElement) {
    ...
    this.artTable = artTableWrapper.querySelector<HTMLDivElement>(
      `.${Classes.artTable}`,
    );
    this.tableHeader = this.artTable.querySelector(
      `:scope > .${Classes.tableHeader}`,
    );
    this.tableBody = this.artTable.querySelector(
      `:scope > .${Classes.tableBody}`,
    );
    this.tableFooter = this.artTable.querySelector(
      `:scope > .${Classes.tableFooter}`,
    );

    const stickyScrollSelector = `.${Classes.artTable} + .${Classes.stickyScroll}`;
    this.stickyScroll =
      artTableWrapper.querySelector<HTMLDivElement>(stickyScrollSelector);
    this.stickyScrollItem = this.stickyScroll.querySelector(
      `.${Classes.stickyScrollItem}`,
    );
  }

  getVirtualTop(): HTMLDivElement {
    return this.tableBody.querySelector<HTMLDivElement>(
      `.${Classes.virtualBlank}.top`,
    );
  }
