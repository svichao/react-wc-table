...
function getRelativeLayoutRect(
  ...
  while (elem != null && elem != base) {
    ...
    deltaY -= accumulateScrollOffset(
      elem.parentElement,
      offsetParent,
      'scrollTop',
    );
    deltaX -= accumulateScrollOffset(
      elem.parentElement,
      offsetParent,
      'scrollLeft',
    );
...
export function getRichVisibleRectsStream(
  ...
  return structureMayChange$.pipe(
    ...
    op.map(() => {
      ...
      const commonOffsetAncestor: HTMLElement | Window =
        findCommonOffsetAncestor(target, scrollParent);
    ...
    op.switchMap(({ scrollParent, commonOffsetAncestor }) => {
      ...
      return events$.pipe(
        op.map((event: unknown) => ({
          ...
          scrollParentRect: getRelativeLayoutRect(
            commonOffsetAncestor,
            scrollParent,
          ),
          event,
