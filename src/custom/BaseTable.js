...
class BaseTable {
  ...
  flattenColumns(columns) {
    ...
    const traverse = (cols) =>
      cols.forEach((col) =>
        col.children?.length ? traverse(col.children) : result.push(col),
      );
    traverse(columns);
    return result;
  }
  ...
  flattenArrayToString(array) {
    const flatten = (arr) =>
      arr.reduce((acc, item) => {
        ...
        return acc.concat(
          isObject(item) && item.v !== undefined ? item.v : item,
        );
      }, []);
  ...
  getColWidth(column) {
    ...
    if (mode === 'custom') {
      ...
      const autoColumnCount = Object.values(config).filter(
        (cfg) => cfg.auto,
      ).length;
    ...
    const applyFrozenToChildren = (column, frozenValue) => {
      if (column.children && column.children.length) {
        column.children = column.children.map((child) =>
          applyFrozenToChildren(child, frozenValue),
        );
      }
  ...
  updateFromChartData({ chartData = {}, style = {}, chartStatus = {}, emit }) {
    ...
    if (chartData && style && chartStatus) {
      this.setColumns(
        this.handleColumns({ chartData, style, chartStatus, emit }),
      );
    }
  ...
  handleColumns({ chartData, style, chartStatus, emit }) {
    ...
    if (chartStatus.drillDownLevel > 0) {
      ...
      if (style?.superTitle?.show) {
        columns = this.generateColumnsByGroup(
          group,
          this.generateColumnsMap(columns),
        );
  ...
  getFieldAlignType(column) {
    if (
      column.from === 'metric' ||
      get(column, 'config.dataType.type') === 'datetime'
    ) {
      return 'right';
    }
    ...
    const filter = (cols) =>
      cols
        .map((col) => {
          ...
          return (col.isGroup && !col.children.length) ||
            col.config?.hideField === 1
            ? null
            : col;
        })
    ...
    newData.forEach((item) => {
      ...
      metricField.forEach((i) => {
        compareOpt.forEach((j) => {
          current[`${i}##${j}`] = this.calcNumber(
            item[i],
            item[`${i}##${j}`],
            compareOn.content === 'percent',
          );
        });
      });
