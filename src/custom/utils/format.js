...
const transNumFormat = (
  value,
  { unit, decimalNum, suffix, prefix, money, prefixString, thousandths },
) => {
...
const transTimeFormat = (value, { timetrans }) => {
  ...
  const time = dayjs.duration(
    value,
    timetrans === '1000' ? 'milliseconds' : 'seconds',
  );
  const minutes = time.minutes();
  const seconds = time.seconds();
  return `${fillZero(parseInt(time.asHours()))}:${fillZero(minutes)}:${fillZero(
    seconds,
  )}`;
};
...
export const getFormat = (columns = []) => {
  ...
  columns.forEach((element) => {
    if (element) {
      if (
        get(element, 'config.dataType.type', element.columnType) ===
        FIELD_TYPE.NUMBER
      ) {
