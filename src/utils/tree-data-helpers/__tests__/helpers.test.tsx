...
test('strategy=parent', () => {
  ...
  expect(helper.getValueAfterToggle('2-1-1')).toEqual([
    '1',
    '2-1-2',
    '2-2',
    '2-3',
  ...
  expect(helper.getValueAfterUncheck('2-1-1')).toEqual([
    '1',
    '2-1-2',
    '2-2',
    '2-3',
...
test('strategy=child', () => {
  ...
  const expected1 = collectNodes([helper.getNode('2')], 'leaf-only').map(
    (n) => n.id,
  );
  expect(actual1.length).toEqual(expected1.length);
  expect(actual1).toEqual(expect.arrayContaining(expected1));
