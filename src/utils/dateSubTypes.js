...
const yearWeekDateDisplayFormat = (isJumpYear = true) => {
  return [
    ...
    },
    {
      ...
      formatter: input => {
        const { startOfWeek, endOfWeek, year, week } = transfromWeekDate(input, isJumpYear);
        return `${year}-${week}周(${startOfWeek.format("MM-DD")}~${endOfWeek.format("MM-DD")})`;
      },
    },
    {
      ...
      formatter: input => {
        const { startOfWeek, endOfWeek, year } = transfromWeekDate(input, isJumpYear);
        return `${year}(${startOfWeek.format("MM/DD")}~${endOfWeek.format("MM/DD")})`;
      },
    },
    {
...
export const getDateGranularity = (dateFormatter = "") => {
  ...
  } else if (["h", "hh", "H", "HH"].some(item => dateFormatter.includes(item))) {
    return list.slice(0, -2);
  } else if (["D", "DD", "d", "dd", "ddd", "dddd"].some(item => dateFormatter.includes(item))) {
    return list.slice(0, -3);
  } else if (["MMMM", "MMM", "MM", "M"].some(item => dateFormatter.includes(item))) {
...
export const getDstFormatData = dstFormat => list.filter(item => item.value === dstFormat)[0];
export const getDateDisplayFormat = (dstFormat, format) => {
  return getDstFormatData(dstFormat)?.dateDisplayFormat.filter(item => item.value === format)[0];
};
