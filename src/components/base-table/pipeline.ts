...
export class TablePipeline {
  ...
  getProps() {
    ...
    Object.keys(this._tableProps || {}).forEach((key) => {
      if (this._tableProps[key]?.length) {
        ...
        if (typeof first === 'function') {
          ...
          result[key] = (...args: any[]) => {
            ...
            if (key === 'rowClassName') {
              const classNames: string[] = this._tableProps[key]
                ?.map((item: any) => {
                  return item(...args);
                })
            ...
            if (key === 'rowRenderer') {
              let { cells } = args[0];
              this._tableProps[key]?.forEach((item: any) => {
  ...
  appendRowEventHandlers(
    ...
    if (!this._rowEventHandlers[eventName]) {
      ...
      const oldFunc = this._rowEventHandlers[eventName];
